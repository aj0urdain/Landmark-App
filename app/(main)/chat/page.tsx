...
const ChatPage = () => {
  ...
  const { data: chatRooms, isLoading: isLoadingRooms } = useQuery({
    ...
    queryFn: async () => {
      ...
      const structuredRooms = parentRooms.map((parent) => {
        ...
        console.log(`🔄 Children for parent ${parent.name}:`, children);
        return {
          ...parent,
          subRooms: children,
        };
      ...
      return structuredRooms.sort((a, b) => {
        ...
        return a.departments.department_name.localeCompare(b.departments.department_name);
      });
    },
  });
  ...
  const unauthorizedDepts =
    chatRooms?.filter(
      (dept) => !userAccess?.departments?.includes(dept.departments.id),
    ) ?? [];
