...
export const textAlgorithm = (type: keyof typeof textProperties) => {
  const queryClient = useQueryClient();
  const { fontSize, fontColor, fontWeight, fontFamily, extraClass, leading } =
    textProperties[type];
  const previewSettings = queryClient.getQueryData(['previewSettings']);
  const zoom = previewSettings?.zoom as number;
  const scale = previewSettings?.scale as number;

  const scaledFontSize = fontSize * scale * zoom;
  const scaledLeading = leading ? leading * scale * zoom : undefined;

  return {
    getTailwind: () => `text-${fontColor} font-${fontFamily} ${extraClass}`.trim(),
    getStyle: () => ({
      fontSize: `${scaledFontSize.toFixed(2)}px`,
      ...(scaledLeading && { lineHeight: `${scaledLeading.toFixed(2)}px` }),
